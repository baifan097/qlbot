        ...
        if len(ck_num) <= 1:
            while goon:
                if V4:
                    ...
                    goon = await ql/data_block(sender)
        elif ck_num.replace(" ", "").isdigit():
            if V4:
                await v4_appoint(ck_num.replace(" ", ""))
            else:
                await ql/data_appoint(ck_num.replace(" ", ""))
...
async def ql/data_block(sender):
    try:
        async with jdbot.conversation(sender, timeout=120) as conv:
            msg = await conv.send_message("请做出您的选择")
            buttons = [
...
async def ql/data_appoint(ck_num):
    msg = await jdbot.send_message(chat_id, f"开始屏蔽账号{ck_num}")
    with open(AUTH_FILE, 'r', encoding='utf-8') as f:
        auth = json.load(f)
    token = auth['token']
