...
from ..diy.utils import ql/data_token, read, write
...
async def myaddrepo(event):
    try:
        ...
        if V4:
            ...
            command = f'ql/data repo {url} "{path}" "{blacklist}" "{dependence}" "{branch}"'
            data = {
                "name": "拉取仓库",
                "command": command,
                "schedule": cron
            }
            res = cron_manage_QL("add", data, ql/data_token(AUTH_FILE))
            if res['code'] == 200:
...
@jdbot.on(events.NewMessage(from_users=chat_id, pattern=r'^ql/data repo'))
async def myql/dataaddrepo(event):
    try:
        if QL:
            SENDER = event.sender_id
            message = event.message.text
            repo = message.replace("ql/data repo", "")
            if len(repo) <= 1:
...
if ch_name:
    jdbot.add_event_handler(myql/dataaddrepo, events.NewMessage(from_users=chat_id, pattern=BOT_SET['命令别名']['cron']))
