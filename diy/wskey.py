...
from ..diy.utils import QL2, ql/data_token, wskey, read, write
...
async def myaddwskey(event):
    try:
        ...
        if len(text) > 1:
            ...
            elif os.path.exists("/ql/data/scripts/wskey_ptkey.py"):
                text += "\n将自动更新cookie列表，自行查看更新情况"
                await cmd("task /ql/data/scripts/wskey_ptkey.py")
            elif os.path.exists("/ql/data/scripts/ql/data_pandaAPI_refreshCK.py") and not os.path.exists("/ql/data/db/wskey.list"):
                text += "\n将自动更新cookie列表，自行查看更新情况"
                await cmd("task /ql/data/scripts/ql/data_pandaAPI_refreshCK.py")
            elif os.path.exists("/ql/data/raw/ql/data_pandaAPI_refreshCK.py") and not os.path.exists("/ql/data/db/wskey.list"):
                text += "\n将自动更新cookie列表，自行查看更新情况"
            elif os.path.exists("/ql/data/scripts/ql/data_pandaAPI_refreshCK.py") and os.path.exists("/ql/data/db/wskey.list"):
                text += "\n由于使用wskey.list存储，无法执行scripts目录下的ql/data_pandaAPI_refreshCK.py脚本"
            elif os.path.exists("/ql/data/raw/ql/data_pandaAPI_refreshCK.py") and os.path.exists("/ql/data/db/wskey.list"):
                text += "\n由于使用wskey.list存储，无法执行raw目录下的ql/data_pandaAPI_refreshCK.py脚本"
