    ...
    try:
        ...
        if V4:
            ...
            if '下载代理' in BOT_SET.keys() and str(BOT_SET['下载代理']).lower() != 'false':
                # os.popen('cd /ql/data && rm -f update.sh* && wget  -q https://git.metauniverse-cn.com/https://raw.githubusercontent.com/curtinlv/gd/main/update.sh >/dev/null && nohup bash update.sh 2>&1 >/ql/data/log/bot/up.log &')
                await cmd('cd /ql/data && rm -f update.sh* && wget  -q https://git.metauniverse-cn.com/https://raw.githubusercontent.com/curtinlv/gd/main/update.sh >/dev/null && nohup bash update.sh 2>&1 >/ql/data/log/bot/up.log &')
            else:
                await cmd('cd /ql/data && rm -f update.sh* && wget  -q https://raw.githubusercontent.com/curtinlv/gd/main/update.sh >/dev/null && nohup bash update.sh 2>&1 >/ql/data/log/bot/up.log &')
                # os.popen('cd /ql/data && rm -f update.sh* && wget  -q https://raw.githubusercontent.com/curtinlv/gd/main/update.sh >/dev/null && nohup bash update.sh 2>&1 >/ql/data/log/bot/up.log &')
